...
declare global {
  ...
  var nwWrldBridge: AppBridge | undefined;
}
...
class SequencerAudio {
  private channelNotes: string[] = ['E2'];
  private lastPlayTime: number = 0;
  private debounceMs: number = 50;
  private lastScheduledTimeMs: number | null = null;
  private kickPlayer: Tone.Player | null = null;
  private isInitializing: boolean = false;
  private isInitialized: boolean = false;
  private isToneStarted: boolean = false;
  ...
  async playChannelBeep(channelNumber: string | number, time?: number): Promise<void> {
    ...
    if (Number.isNaN(numericChannel) || numericChannel < 1) {
