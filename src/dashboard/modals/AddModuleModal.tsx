...
import type { UserData, TrackId, SetId, ModuleMetadata } from "@/types";
...
export const AddModuleModal: React.FC<AddModuleModalProps> = ({
  ...
  const [hoveredPreviewModuleId, setHoveredPreviewModuleId] = useState<
    string | null
  >(null);
  const [loadingPreviewModuleId, setLoadingPreviewModuleId] = useState<
    string | null
  >(null);
  const previewRequestRef = useRef<{
    moduleId: string | null;
    requestId: string | null;
  }>({ moduleId: null, requestId: null });
  ...
  const handleAddToTrack = (module: ModuleMetadata) => {
  ...
  const modulesByCategory = modulesWithTrackIndicator.reduce(
    (acc: Record<string, ModuleMetadata[]>, module) => {
  ...
  return (
    <Modal
      ...
      size="medium"
    >
      <div className="mb-4 pb-4 border-b border-neutral-800 bg-[#101010] flex justify-between items-baseline">
        <span className="uppercase text-neutral-300 relative inline-block flex items-center gap-2">
              ...
              <div className="pl-6 uppercase flex flex-col flex-wrap gap-2">
                {modules.map((module) => {
                  const handlePreview = () => {
                    ...
                    sendToProjector("preview-module" as any, previewData.props);
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Button
                          onClick={(e) => {
                            e.stopPropagation();
                            onEditModule && onEditModule(module.id || module.name);
                          }}
                          type="secondary"
                          icon={<FaCode />}
                          title="Edit code"
