...
interface ColumnVisibilityMethodOptions {
  leftColumn?: boolean;
  rightColumn?: boolean;
}
...
class CodeColumns extends ModuleBase {
  ...
  leftColumn!: HTMLDivElement | null;
  rightColumn!: HTMLDivElement | null;
  ...
  generateInitialContent(): void {
    ...
    const getRandomSnippets = (snippets: string[], count: number): string => {
  ...
  columnVisibility({ leftColumn = true, rightColumn = true }: ColumnVisibilityMethodOptions = {}): void {
    if (this.leftColumn) {
  ...
  setColumnVisibility(options: ColumnVisibilityMethodOptions = {}): void {
