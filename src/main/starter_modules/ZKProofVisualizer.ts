...
class ZKProofVisualizer extends ModuleBase {
  ...
  match({ matchCount = 6 }: MatchOptions = {}): void {
    ...
    const matches: Array<[HTMLElement, HTMLElement]> = [];
    ...
    while (
      ...
      if (wordText && !usedWords.has(wordText)) {
        ...
        if (pairElement && pairText) {
          const wordElem = word as HTMLElement;
          const pairElem = pairElement as HTMLElement;
          matches.push([wordElem, pairElem]);
