...
declare const loadJson: (path: string) => Promise<unknown>;
...
const clampNumber = (v: unknown, min: number, max: number): number => {
  const n = Number(v);
  if (!Number.isFinite(n)) return min;
  return Math.min(max, Math.max(min, n));
};

const toNumericSeries = (raw: unknown, valueKey: string | null): number[] => {
...
const sampleSeries = (series: unknown, maxPoints: number): number[] => {
  const arr = Array.isArray(series) ? series : [];
  const max = Math.floor(Number(maxPoints) || 0);
  if (max <= 0 || arr.length <= max) return arr as number[];
  const out: number[] = [];
...
class PerlinBlob extends ModuleBase {
  ...
  init(): void {
    ...
    const sketch = (p: p5) => {
      ...
      p.draw = () => {
        ...
        let previousLayerRadii: Record<number, number> = [];
        ...
        for (let i = 0; i < numLayers; i++) {
          ...
          const currentLayerRadii: Record<number, number> = [];
          p.beginShape();
  ...
  async loadData({
    ...
    const parsed = sampleSeries(toNumericSeries(raw, valueKey ?? null), safeMax);
    if (parsed.length > 0) {
