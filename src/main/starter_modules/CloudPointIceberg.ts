...
interface NoiseConstructor {
  new (seed: number): Noise;
}

declare const Noise: NoiseConstructor;
...
class CloudPointIceberg extends BaseThreeJsModule {
  ...
  noise!: Noise;
  lastColorChoice!: number;

  constructor(container: HTMLElement) {
    super(container);
    if (!THREE || !Noise) return;

    this.name = "CloudPointIceberg";
  ...
  destroy(): void {
    ...
    if (this.wireMesh) {
      ...
      if (this.wireMesh.material) {
        if (Array.isArray(this.wireMesh.material)) {
          this.wireMesh.material.forEach(m => m.dispose());
        } else {
