...
class BasicGeometry extends BaseThreeJsModuleType {
  // Module properties
  customGroup!: THREE.Group | null;
  line!: THREE.Line | null;
  lineSegments!: THREE.LineSegments | null;
  destroyed!: boolean;
  setCustomAnimate!: (fn: () => void) => void;
  ...
  shape({ shape = "cylinder" }: { shape?: string } = {}): void {
    if (this.destroyed) return;
  ...
  shapeGeometry(shapeType: string): THREE.BufferGeometry {
  ...
  cylinder(radiusTop: number, radiusBottom: number, height: number, radialSegments: number, heightSegments?: number): THREE.BufferGeometry {
  ...
  cone(radius: number, height: number, radialSegments: number): THREE.BufferGeometry {
  ...
  octahedron(radius: number, detail: number): THREE.BufferGeometry {
  ...
  tetrahedron(radius: number, detail: number): THREE.BufferGeometry {
  ...
  latheGeometry(pointsCount: number, segments: number, phiStart: number, phiLength: number): THREE.BufferGeometry {
