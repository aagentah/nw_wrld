...
const sample = <T>(arr: T[]): T | null => {
  if (!arr || !Array.isArray(arr) || arr.length === 0) return null;
  return arr[Math.floor(Math.random() * arr.length)];
};

class CubeCube extends BaseThreeJsModuleType {
  // Module properties
  ...
  animateLoop(): void {
    ...
    this.cubeGrid.forEach((cube: THREE.Mesh) => {
      if (cube.children.length > 0) {
        cube.children.forEach((particleGroup: THREE.Group) => {
          particleGroup.children.forEach((particle: THREE.Mesh) => {
  ...
  appendCube({ duration }: { duration?: number } = {}): void {
    if (this.destroyed) return;

    const randomCube = sample(this.cubeGrid);
    if (randomCube) {
      const material = randomCube.material as THREE.MeshBasicMaterial;
      ...
      if (duration > 0) {
        setTimeout(() => {
          ...
          particleGroup.traverse((child: THREE.Mesh) => {
  ...
  resetCubes(): void {
    ...
    this.cubeGrid.forEach((cube: THREE.Mesh) => {
      ...
      material.opacity = Math.random() > 0.8 ? 0.1 : 0;
  ...
  destroy(): void {
    ...
    if (this.cubeGroup) {
      this.cubeGroup.traverse((child: THREE.Mesh) => {
        if (child.geometry) child.geometry.dispose();
        if (child.material) {
