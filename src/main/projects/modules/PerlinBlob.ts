...
const toNumericSeries = (raw: any, valueKey: string): number[] => {
...
const sampleSeries = (series: number[], maxPoints: number): number[] => {
...
class PerlinBlob extends ModuleBaseType {
  ...
  noiseOffset!: number;
  noiseScale!: number;
  blobPoints!: Array<{x: number; y: number; z: number}>;
  isAnimating!: boolean;
  name!: string;
  elem!: HTMLElement;
  data!: any;
