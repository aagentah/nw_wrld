// src/projector/helpers/moduleBase.ts

import type { MethodDefinition, MethodOptionDefinition } from '../../types/moduleMethods.js';
...
export interface RandomZoomResult {
  scale: number;
  x: number;
  y: number;
}
...
/**
 * ModuleBase - Base class for all projector modules
 *
 * Provides essential lifecycle management for containers (HTML elements).
 * Handles initialization of the container, transformation states (position, scale, opacity),
 * and applies necessary styles to prepare the element for manipulation.
 *
 * The destroy method ensures proper cleanup of the element by removing it from the DOM
 * and clearing any references to avoid memory leaks.
 */
export class ModuleBase {
  ...
  viewportLineElem: SVGElement | null;
  ...
  constructor(container: HTMLElement | null) {
    ...
    if (this.elem) {
      ...
      this.elem.style.opacity = this.currentOpacity.toString();
      this.updateTransform();
    }
  }

  show(options: ModuleBaseOptions = {}): void {
  ...
  hide(options: ModuleBaseOptions = {}): void {
    const { duration = 0 } = options;
    const moduleElem = this.elem;
   ...
   * @param options - X and Y offset percentages
   */
  offset(options: ModuleBaseOptions = {}): void {
   ...
   * @param options - Scale factor
   */
  scale(options: ModuleBaseOptions = {}): void {
   ...
   * @param options - Opacity value between 0 and 1
   */
  opacity(options: ModuleBaseOptions = {}): void {
    const { opacity: opacityValue = 1 } = options;
    const moduleElem = this.elem;

    if (moduleElem) {
      // Clamp the opacity value between 0 and 1
      const clampedValue = Math.min(Math.max(opacityValue, 0), 1);
      this.currentOpacity = clampedValue;
      moduleElem.style.opacity = this.currentOpacity.toString();
    } else {
      console.warn(`Module instance does not have an 'elem' property.`);
    }
  }
   ...
   * @param options - Direction, speed, and duration
   */
  rotate(options: ModuleBaseOptions = {}): void {
   ...
   * @param options - Scale range and position
   * @returns The applied transformation values
   */
  randomZoom(options: ModuleBaseOptions = {}): RandomZoomResult | undefined {
   ...
   * @param options - Target position and line properties
   */
  viewportLine(options: ModuleBaseOptions = {}): void {
    const { x = 50, y = 50, length = 100, opacity: opacityValue = 1 } = options;
    const moduleElem = this.elem;
    ...
    if (this.viewportLineElem) {
      ...
      if (this.externalElements && Array.isArray(this.externalElements)) {
        const index = this.externalElements.indexOf(this.viewportLineElem as any);
        if (index > -1) {
    ...
    // Find closest side
    const closestSide = (Object.keys(distances) as Array<'top' | 'right' | 'bottom' | 'left'>).reduce((a, b) =>
    ...
    let startX: number;
    let startY: number;
    let outwardDirX: number;
    let outwardDirY: number;
    ...
    path.setAttribute("stroke-opacity", clampedOpacity.toString());
    ...
    this.externalElements.push(svg as any);
   ...
   * @param options - Color value
   */
  background(options: ModuleBaseOptions = {}): void {
   ...
   * @param options - Duration to apply inversion
   */
  invert(options: ModuleBaseOptions = {}): void {
    const { duration = 0 } = options;
    const moduleElem = this.elem;
