// src/projector/helpers/threeBase.ts
...
import { ModuleBase, type ModuleBaseOptions } from "./moduleBase";
import { animationManager } from "./animationManager";
import type { MethodDefinition } from "../../types/moduleMethods";
...
export type ViewDirectionType = "front" | "top" | "right" | "back" | "bottom" | "left";

export class BaseThreeJsModule extends ModuleBase {
  static methods: MethodDefinition[] = [
    ...ModuleBase.methods,
    {
      name: "zoomLevel",
      executeOnLoad: true,
  ...
  currentAnimation: CameraAnimationType | null;
  ...
  constructor(container: HTMLElement | null) {
    ...
    this.camera = new THREE.PerspectiveCamera(
      80,
      this.elem ? this.elem.offsetWidth / this.elem.offsetHeight : 1,
      0.1,
      1000
    );
   ...
   * @param model - The 3D model to add to the scene.
   */
  setModel(model: THREE.Object3D): void {
    ...
    if (this.controls && this.modelCenter) {
  ...
  configureCamera(): void {
    ...
    this.updateCameraViewDirection(this.cameraSettings.viewDirection as ViewDirectionType);
   ...
   * @param callback - The custom animation function.
   */
  setCustomAnimate(callback: () => void): void {
   ...
   */
  snapCameraToRandomAngle(): void {
    if (!this.modelCenter || !this.camera || !this.controls) {
   ...
   * @param delta - The angle in radians to roll the camera.
   */
  rollCamera(delta: number): void {
    if (!this.renderer || !this.scene || !this.camera || this.destroyed) return;
   ...
   * @param delta - The amount to pan.
   */
  panCameraForward(delta: number): void {
    if (!this.renderer || !this.scene || !this.camera || this.destroyed) return;
   ...
   * @param delta - The amount to pan.
   */
  panCameraLeft(delta: number): void {
    if (!this.renderer || !this.scene || !this.camera || this.destroyed) return;
   ...
   */
  zoomLevel(options: ModuleBaseOptions): void {
    if (!this.renderer || !this.scene || !this.camera || this.destroyed) return;

    const { zoomLevel = this.cameraSettings.zoomLevel } = options;

    this.cameraSettings.zoomLevel = THREE.MathUtils.clamp(zoomLevel as number, 0, 100);
   ...
   * @param options - Configuration options.
   */
  cameraAnimation(options: ModuleBaseOptions): void {
    if (!this.renderer || !this.scene || !this.camera || this.destroyed) return;

    const { cameraAnimation = this.cameraSettings.cameraAnimation } = options;

    this.cameraSettings.cameraAnimation = cameraAnimation as string;

    this.startCameraAnimation(
      this.cameraSettings.cameraAnimation as CameraAnimationType,
   ...
   * @param percentage - The zoom level percentage.
   */
  updateCameraZoom(percentage: number): void {
    if (!this.renderer || !this.scene || !this.camera || this.destroyed) return;
   ...
   */
  updateCameraViewDirection(direction: ViewDirectionType): void {
    if (!this.renderer || !this.scene || !this.camera || this.destroyed) return;

    if (!this.modelBoundingBox || !this.modelCenter) {
      console.warn("Model bounding box not loaded yet.");
      return;
    }
   ...
   * @param animation - The type of camera animation.
   * @param speed - The speed of the animation.
   */
  startCameraAnimation(animation: CameraAnimationType, speed: number): void {
    if (!this.renderer || !this.scene || !this.camera || this.destroyed) return;
   ...
   * @param deltaAzimuth - The change in azimuthal angle.
   * @param deltaPolar - The change in polar angle.
   */
  ...
  destroy(): void {
    const disposeObject = (object: any): void => {
      try {
        ...
        if (object.animations) {
          object.animations.forEach((clip: any) => clip.dispose());
        }
    ...
    for (let propName in this) {
      if (this[propName as keyof this] && typeof this[propName as keyof this] === "object") {
        disposeObject(this[propName as keyof this]);
